...
|---|---|---|
| #1 | `make demo` renders the fixture network on first load | **Verified & closed** — `docker compose --profile demo up --build`, headless-Chromium render shows all nodes/edges, zero console errors |
...
| #8 | Zero browser console errors; `/api/health` 200 | **Verified & closed** — re-confirmed against the real 15-device graph render |
| #9 | Docker image builds and `make demo` comes up | **Verified & closed** — two-stage build, `/api/health` 200, `/api/graph` 8 nodes / 7 edges, SPA + assets load |
| #10 | External nodes carry edge labels | Open — real LAN adds weight: one Gen1 fans 15 unlabeled `other` edges to a single external target (comment on #10) |

New bugs from real-LAN testing (filed 2026-08-30, milestone Phase 1):

| Issue | Title | Labels |
|---|---|---|
| [#11](https://github.com/pedrobsm/shelly_mesh_manager/issues/11) | Startup scan bypasses ScanManager — concurrent scans, no 409 on boot | `bug` `phase-1` |
| [#12](https://github.com/pedrobsm/shelly_mesh_manager/issues/12) | Gen1 config snapshot churns on every scan (volatile `/settings` fields) | `bug` `phase-1` `needs-hardware` |
| [#13](https://github.com/pedrobsm/shelly_mesh_manager/issues/13) | Resolver classifies non-`turn` Shelly path as `external` not `unknown_shelly` | `bug` `phase-1` `spec` |

Comments with the real-LAN results were added to #2, #3, #5, #6, #7 and #10.
Closed as verified: #1, #8, #9. Still open: #2 (Gen3 + range-scan timing),
#3 (pending #13), #4 (no auth device), #5 (optional physical unplug), #6
(40-device timing), #7 (fix tracked in #12), #10 (label decision).
